...
def prediction():
    ...
    if st.button("Predict"):
        ...
        if result==1:
            ...
            trace = go.Pie(labels=labels, values=value,textfont=dict(size=20),marker=dict(colors=colors))
        ...
        if result==0:
            ...
            trace = go.Pie(labels=labels, values=values,textfont=dict(size=20),marker=dict(colors=colors))
            fig=go.FigureWidget(data=[trace])
            st.plotly_chart(fig)
